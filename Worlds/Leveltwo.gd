...
const TIME_WARP = 0.15 # Factor in which time warps
const REWIND_FRAMERATE = 0.005
...
func _process(delta):
	if Input.is_action_just_pressed('player_rewind') || Input.is_action_just_pressed('ui_right') || Input.is_action_just_pressed('ui_left') || Input.is_action_just_pressed('ui_down') || Input.is_action_just_pressed('ui_up'):
		Engine.time_scale = 1.0
	elif Input.is_action_just_released('player_rewind') || Input.is_action_just_released('ui_right') || Input.is_action_just_released('ui_left') || Input.is_action_just_released('ui_down') || Input.is_action_just_released('ui_up'):
		Engine.time_scale = TIME_WARP
	counter += delta
